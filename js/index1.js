...
const getUnitSpecs = (nUnits) => State((documentBody) => {
    ...
    // spec :: UnitSpec
    const spec = UnitsSpec(unitSideLength - 0.35, unitsWide, unitsTall);
...
const renderImg = (url, units) => {
    ...
    const onload = function () {
        // Purposefully reverse nT & nW for glitch effect
        this.width = nT;
        this.height = nW;
...
const showError = (units) => (error) => {
    const err = error.toString().replace(/\s/g, '*') + '*';
    const flatUnits = units.flat();
